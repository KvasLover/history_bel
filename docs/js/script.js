document.addEventListener('DOMContentLoaded', function() {
    ...
    // Переход по адресу на карту
    const addressLinks = document.querySelectorAll('.specific-address-link');
    ...
    addressLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            const mapUrl2 = `geo:0,0?q=${encodedAddress}`; 

            window.open(mapUrl, '_blank');
            window.location.href = mapUrl2;
        });
    });
});